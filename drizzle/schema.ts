import { pgTable, index, foreignKey, varchar, numeric, timestamp, text, primaryKey, integer } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"



export const ojirTransaction = pgTable("ojir_transaction", {
	...
	index("transaction_date_idx").using("btree", table.transactionDate.asc().nullsLast().op("timestamptz_ops")),
	index("transaction_ref_idx").using("btree", table.transactionRefNo.asc().nullsLast().op("text_ops")),
...
export const ojirCalendarEvent = pgTable("ojir_calendar_event", {
	id: varchar({ length: 255 }).primaryKey().notNull(),
	...
	index("calendar_event_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [ojirUser.id],
...
export const ojirSession = pgTable("ojir_session", {
...
export const ojirVerificationToken = pgTable("ojir_verification_token", {
	identifier: varchar({ length: 255 }).notNull(),
	token: varchar({ length: 255 }).notNull(),
	expires: timestamp({ withTimezone: true, mode: 'string' }).notNull(),
}, (table) => [
	primaryKey({ columns: [table.identifier, table.token], name: "ojir_verification_token_identifier_token_pk"}),
]);

export const ojirAccount = pgTable("account", {
	userId: varchar("user_id", { length: 255 }).notNull(),
	...
	primaryKey({ columns: [table.provider, table.providerAccountId], name: "ojir_account_provider_provider_account_id_pk"}),
]);
