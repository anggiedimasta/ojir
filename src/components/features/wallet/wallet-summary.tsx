...
export function WalletSummary({ summary, formatCurrency }: WalletSummaryProps) {
  ...
  const IncomeIcon = () => <TrendingUp className="w-6 h-6 text-green-600" />;
  const ExpenseIcon = () => <TrendingDown className="w-6 h-6 text-red-600" />;
  const NetIncomeIcon = () => <Wallet className={`w-6 h-6 ${isNetPositive ? 'text-green-600' : 'text-red-600'}`} />;
  const TransactionIcon = () => <CreditCard className="w-6 h-6 text-purple-600" />;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <SummaryCard
        ...
        icon={NetIncomeIcon}
        gradient={isNetPositive ? "bg-gradient-to-br from-green-50 to-emerald-50" : "bg-gradient-to-br from-red-50 to-rose-50"}
