...
export function TransactionItem({
  ...
  const isVirtualAccount = (transaction: TransactionResponse) => {
    return transaction.transactionType === "virtual-account";
  };
    ...
    <div className="group p-4 transition-all duration-200 hover:bg-slate-50/50">
      ...
      <div className="mb-2 flex items-center justify-between">
        <div className="flex flex-wrap items-center gap-2">
          {isBIFastTransfer(transaction) ? (
            ...
            <span className="rounded-full bg-cyan-100 px-2 py-1 font-medium text-cyan-700 text-xs">
              Top-up
            </span>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4 text-slate-500 text-xs">
          ...
          {/* Show virtual account number if available */}
          {isVirtualAccount(transaction) &&
            transaction.recipientBankAccount && (
              <span className="font-mono text-slate-400">
                VA: {transaction.recipientBankAccount}
              </span>
            )}
        </div>
