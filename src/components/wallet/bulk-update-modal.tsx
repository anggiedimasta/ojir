...
export function BulkUpdateModal({
	...
	const transactionCount =
		pendingWalletChange && transactions
			? transactions.filter(
					(t) => t.walletId && pendingWalletChange.fromIds.includes(t.walletId),
				).length
			: 0;

	const targetWalletName = pendingWalletChange
		? wallets.find((w) => w.id === pendingWalletChange.toId)?.name ||
			"the selected wallet"
		: "";
				...
				<div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
					<div className="mb-2 flex items-center gap-2">
						...
						<h4 className="font-medium text-blue-900">
							Confirm Transaction Move
						</h4>
					</div>
					<p className="text-blue-700 text-sm">
						{pendingWalletChange && (
							<>
								Move <strong>{transactionCount}</strong> transactions from the
								current wallet selection to <strong>{targetWalletName}</strong>?
							</>
						)}
					</p>
				</div>
