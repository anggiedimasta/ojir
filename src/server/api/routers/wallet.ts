...
export const walletRouter = createTRPCRouter({
  ...
  createWallet: protectedProcedure
    .input(
      z.object({
        ...
        accountNumber: z.string().min(4).max(4).regex(/^\d{4}$/, "Account number must be exactly 4 digits").optional(),
  ...
  getOrCreateUncategorizedWallet: protectedProcedure
    .mutation(async ({ ctx }) => {
  ...
  updateWallet: protectedProcedure
    .input(
      z.object({
        ...
        type: z.enum(['debit', 'credit', 'savings', 'current', 'investment']).optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const updateData: any = {
        ...
        ...(input.balance !== undefined && { balance: input.balance.toString() }),
  ...
  getTransactions: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.sortBy) {
        switch (input.sortBy) {
          case 'amount':
            orderBy = input.sortOrder === 'asc' ? asc(transactions.amount) : desc(transactions.amount);
            break;
          case 'recipient':
            orderBy = input.sortOrder === 'asc' ? asc(transactions.recipient) : desc(transactions.recipient);
            break;
          case 'date':
          default:
            orderBy = input.sortOrder === 'asc' ? asc(transactions.transactionDate) : desc(transactions.transactionDate);
            break;
        }
      }
  ...
  getSummary: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      userTransactions.forEach(t => {
        if (t.recipient) {
          const existing = merchantTotals.get(t.recipient) || { totalAmount: 0, count: 0 };
          // Only count expenses for top merchants (spending analysis)
          if (t.direction === 'out') {
            existing.totalAmount += parseFloat(t.amount);
            existing.count += 1;
  ...
  syncTransactions: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (userWallets.length === 0) {
        throw new Error("You must create at least one wallet before syncing emails. Please add a wallet first.");
      }
        ...
        for (const message of messages) {
          try {
            ...
            if (parsedTransaction.sourceOfFund) {
              ...
              const sourceAccountNumber = extractAccountNumber(parsedTransaction.sourceOfFund);

              const walletAccountInSource = userWalletAccountNumbers.some(wallet =>
            ...
            // Debug specific transaction
              ...
              skippedMessages.push({ id: message.id, reason: "already_exists" });
              continue;
            }
            ...
            if (parsedTransaction.sourceAccount) {
              ...
              const accountNumber = parsedTransaction.sourceAccount.replace(/\*/g, '').slice(-4);
            ...
            if (!targetWallet[0] && parsedTransaction.bankSender && parsedTransaction.sourceAccount) {
              const accountNumber = parsedTransaction.sourceAccount.replace(/\*/g, '').slice(-4);
              ...
              if (accountNumber.length === 4) {
                targetWallet = await ctx.db
                  ...
                  .where(
                    and(
                      eq(wallets.userId, ctx.session.user.id),
                      eq(wallets.bankCode, parsedTransaction.bankSender.toLowerCase()),
                      eq(wallets.accountNumber, accountNumber)
                    )
                  )
                  .limit(1);
            ...
            const newTransaction = await ctx.db.insert(transactions).values({
              userId: ctx.session.user.id,
  ...
  clearAllTransactions: protectedProcedure
    .mutation(async ({ ctx }) => {
      const result = await ctx.db
        .delete(transactions)
  ...
  enableAutoSync: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        const topicName = input.topicName || (process.env.GOOGLE_CLOUD_PROJECT ? `projects/${process.env.GOOGLE_CLOUD_PROJECT}/topics/gmail-notifications` : 'default-topic');

        const result = await setupGmailPushNotifications(ctx.accessToken, topicName);
        ...
        throw new Error(`Failed to enable auto sync: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }),
  ...
  disableAutoSync: protectedProcedure
    .mutation(async ({ ctx }) => {
      ...
      try {
        ...
        throw new Error(`Failed to disable auto sync: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }),

  // Remap existing transactions to correct wallets based on account number
  remapTransactionsToWallets: protectedProcedure
    .mutation(async ({ ctx }) => {
      try {
        // Get all user's wallets with account numbers
        const userWallets = await ctx.db
