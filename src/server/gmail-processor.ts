...
async function processUserEmails(userId: string, accessToken: string, historyId: string) {
  try {
      ...
      if (historyRecord.messagesAdded) {
        for (const messageAdded of historyRecord.messagesAdded) {
          if (messageAdded.message?.id) {
            await processSingleEmail(userId, accessToken, messageAdded.message.id);
          }
        }
      }
    }
...
async function processSingleEmail(userId: string, accessToken: string, messageId: string) {
  try {
...
async function saveTransaction(userId: string, parsedTransaction: any, messageId: string) {
  try {
